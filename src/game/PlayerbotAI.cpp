    ...
    switch (packet.GetOpcode())
    {
        ...
        case SMSG_FORCE_RUN_SPEED_CHANGE:
        {
            ...
            if (guid != master->GetGUID())
                return;
            if (master->IsMounted() && !m_bot->IsMounted())
            {
            ...
            else if (!master->IsMounted() && m_bot->IsMounted())
            {
        ...
        {
			aiRegistry->GetSocialManager()->AcceptInvitation();
            return;
        }
        ...
        case SMSG_TRADE_STATUS:
        {
            ...
            if (status == 4)
				aiRegistry->GetSocialManager()->AcceptTrade();

            //1 == TRADE_STATUS_BEGIN_TRADE
            else if (status == 1)
				aiRegistry->GetSocialManager()->BeginTrade();
...
int PlayerbotAI::UpdateIgnoreTime(uint32 spellId)
{
    ...
    if (pSpell->IsChannelActive() || pSpell->IsAutoRepeat())
        return (GetSpellDuration(pSpell->m_spellInfo) / 1000) + GLOBAL_COOLDOWN;
...
bool PlayerbotAI::isDrink(const ItemPrototype* pItemProto, const void* param)
{
    return (pItemProto->Class == ITEM_CLASS_CONSUMABLE && pItemProto->SubClass == ITEM_SUBCLASS_FOOD && 
        pItemProto->Spells[0].SpellCategory == 59);
}

bool PlayerbotAI::isFood(const ItemPrototype* pItemProto, const void* param)
{
    return (pItemProto->Class == ITEM_CLASS_CONSUMABLE && pItemProto->SubClass == ITEM_SUBCLASS_FOOD && 
        pItemProto->Spells[0].SpellCategory == 11);
}

Item* PlayerbotAI::FindUsableItem(bool predicate(const ItemPrototype*, const void*), const void* param, int *count/*=NULL*/) const
{
	Item* found = NULL;
...
bool PlayerbotAI::isBandage(const ItemPrototype* pItemProto, const void* param)
{
    return pItemProto->Class == ITEM_CLASS_CONSUMABLE && pItemProto->SubClass == ITEM_SUBCLASS_BANDAGE;
}

bool PlayerbotAI::isPoison(const ItemPrototype* pItemProto, const void* param)
{
...
void PlayerbotAI::HandleCommand(const std::string& text, Player& fromPlayer)
{
    ...
    {
		aiRegistry->GetSocialManager()->Trade(text.c_str());
    }

    else if (text == "report")
	{
        aiRegistry->GetInventoryManager()->ListQuestItems();
	}

    else if (text == "attack")
    {
		aiRegistry->GetMoveManager()->Attack(ObjectAccessor::GetUnit(*m_bot, fromPlayer.GetSelection()));
    }
    else if (text == "leave")
    {
		aiRegistry->GetSocialManager()->LeaveGroup();
    }
	else if(text == "release" && !m_bot->isAlive())
	{
		aiRegistry->GetMoveManager()->ReleaseSpirit();
	}
    ...
    {
		aiRegistry->GetInventoryManager()->UseItem(text.c_str());
    }

    else if (text.size() > 2 && text.substr(0, 2) == "e " || text.size() > 6 && text.substr(0, 6) == "equip ")
    {
		aiRegistry->GetInventoryManager()->EquipItem(text.c_str());
    }
    
	else if (text == "quests")
	{
		aiRegistry->GetQuestManager()->ListQuests();
	}
	
	else if (text.size() > 5 && text.substr(0, 5) == "drop ")
	{
		aiRegistry->GetQuestManager()->DropQuest(text.substr(5).c_str());
	}
    
    else if (text == "spells")
    {
		aiRegistry->GetSpellManager()->ListSpells();
    }
    ...
    {
		aiRegistry->GetInventoryManager()->Reward(text.c_str());
    }
}
